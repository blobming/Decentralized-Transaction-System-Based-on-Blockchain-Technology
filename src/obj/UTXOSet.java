...
public class UTXOSet {
	...
	/*
	 * previous Version public static HashMap<String, HashSet<Vout>>
	 * FindSpendableOutputs(String address, double amount) { HashMap<String,
	 * HashSet<Vout>> unspentOutputs = new HashMap<>(); double accumulated = 0;
	 * for(Entry<String, Object> entry : Global.utxoDB.getAllKey().entrySet()) {
	 * HashSet<Vout> vouts = (HashSet<Vout>) entry.getValue(); for(Vout vout :
	 * vouts) { if(vout.IsLockedWithKey(Utilities.hashKeyForDisk(address)) &&
	 * accumulated < vout.getValue()) { accumulated += vout.getValue();
	 * if(!unspentOutputs.containsKey(entry.getKey())) {
	 * unspentOutputs.put(entry.getKey(), new HashSet<Vout>()); }
	 * unspentOutputs.get(entry.getKey()).add(vout); } } } return unspentOutputs; }
	 */
	...
	/*
	 * if isCoinbaseTX continue; for each vin: if(vin.txid is in UTXO && vin.voutNum
	 * is in utxo[vin.txid]) remove it for each vout: add into UTXO
	 */
	public static void UpdateBackup(BlockBody blockbody) {
	...
	public static void Update(BlockBody blockbody) {
		...
		for(Transaction tx : txs) {
			if(!tx.isCoinBase()) {
				System.out.println("not coinbase");
				if(Global.utxoDB.get(tx.getTxid()) != null)	continue;
				Vin[] vins = tx.getVin();
				for (int i = 0; i < vins.length; i++) {
					HashSet<Vout> outputs = (HashSet<Vout>) Global.utxoDB.get(vins[i].getTxid());
					HashSet<Vout> newOutputs = new HashSet<>();
	...
	public static void addTempTX(Transaction tx) {
		Vin[] vins = tx.getVin();
		for (int i = 0; i < vins.length; i++) {
			HashSet<Vout> outputs = (HashSet<Vout>) Global.utxoDB.get(vins[i].getTxid());
			HashSet<Vout> newOutputs = new HashSet<>();
...
//			}else {
			Global.utxoDB.put(vins[i].getTxid(), newOutputs);
			/*
			 * 不删除vins[i].getTxid()的原因是防止在vins[i].getTxid()还没被写入区块前，就在utxo里被删除了
			 * 这样的话在根据区块更新utxo时会检索不到vins[i].getTxid()
			 */
//			}
		}
	...
	/*
	 * for every block: for every transaction: For Vin: if vin.transactionId in UTXO
	 * -->remove add into spentTXOs <vin.transactionId, vin.voutNum> For Vout: if
	 * (thisTransaction.Id in spentTXOs && vout is in
	 * spentTXOs.get(thisTransactionId)): //其实可以不要这段代码 remove from UTXO else: add
	 * into UTXO
	 */
